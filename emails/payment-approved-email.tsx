...
export function PaymentApprovedEmail({ name, plan, dashboardUrl }: { name: string; plan: string; dashboardUrl: string }) {
  return (
    <EmailShell
      preview={`Pagamento aprovado. Plano ${plan.toUpperCase()} liberado no vetorcad.`}
    ...
    >
      <EmailParagraph>
        {name ? `${name}, seu pagamento foi aprovado.` : "Seu pagamento foi aprovado."} O plano {plan.toUpperCase()} já foi sincronizado com sua conta vetorcad.
      </EmailParagraph>
      <EmailParagraph>
