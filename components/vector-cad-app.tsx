...
import { exportImageToPdf, type PdfExportProgress, type PdfOrientation, type PdfPageSize } from "@/lib/exporters/pdf";
...
function Internal3DOnlyView({ document, svg, fileName, unit, onBack }: { document: VectorDocument | null; svg: string; fileName: string; unit: Unit; onBack: () => void }) {
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_50%_-20%,#1d3428_0,#080c0b_42%)] text-[#e8efeb]">
    ...
    <section className="mx-auto max-w-[1600px] p-4 pt-24 md:p-7 md:pt-28">
      {document ? <SvgTo3DCadViewer document={document} svg={svg} fileName={fileName} unit={unit} /> : <div className="grid min-h-[300px] place-items-center rounded-2xl border border-[#26312c] bg-[#101613] text-sm text-[#dce8e1]">Vetorize uma imagem antes de abrir a visualização 3D.</div>}
    </section>
  </main>;
}
...
function VectorInspectionOverlay({ doc, showContours, showLayers, onSelect }: { doc: VectorDocument | null; showContours: boolean; showLayers: boolean; onSelect: (path: VectorDocument["paths"][number], index: number) => void }) {
  if (!doc || !showContours) return null;
  const colors = { CONTOURS: "#b7f34a", DETAILS: "#54a9ff", GUIDES: "#ffd34d" } as const;
  return <svg viewBox={`0 0 ${doc.sourceWidth || doc.width} ${doc.sourceHeight || doc.height}`} className="pointer-events-none absolute inset-0 h-full w-full overflow-visible">
    {doc.paths.map((path, index) => {
      if (path.points.length < 2) return null;
      const d = path.points.map((point, pointIndex) => `${pointIndex ? "L" : "M"}${point.x} ${point.y}`).join(" ") + (path.closed ? " Z" : "");
      return <path key={`${path.layer}-${index}`} d={d} fill={path.closed ? `${colors[path.layer]}12` : "none"} stroke={showLayers ? colors[path.layer] : "#b7f34a"} strokeWidth={Math.max(0.8, (doc.sourceWidth || doc.width) / 900)} vectorEffect="non-scaling-stroke" className="pointer-events-auto cursor-pointer transition-opacity hover:opacity-70" onMouseEnter={() => onSelect(path, index)} onClick={() => onSelect(path, index)} />;
    })}
  </svg>;
}
...
export function VectorCadApp({ onUsageChange, initialData, onProjectChange, projectId, persistenceStatus = "saved", projectVersion = 0, savedProjectVersion = 0 }: { onUsageChange?: (usage: UsageInfo) => void; initialData?: CadProjectData | null; onProjectChange?: (data: CadProjectData) => void; projectId?: string | null; persistenceStatus?: "saved" | "saving" | "dirty" | "error"; projectVersion?: number; savedProjectVersion?: number }) {
  ...
  const [showPreviewContours, setShowPreviewContours] = useState(true);
  const [showPreviewLayers, setShowPreviewLayers] = useState(true);
  const [showPreviewTexts, setShowPreviewTexts] = useState(false);
  const [selectedPreviewPath, setSelectedPreviewPath] = useState<{ path: VectorDocument["paths"][number]; index: number } | null>(null);
  ...
  const handleDownloadImage = () => {
    ...
    link.download = "vetorcad-imagem-melhorada.png";
  ...
  const handleExportPdf = async () => {
    const improvedImage = aiEnhancedPreview;
    const processedImageSource = processedPreview || processedImage;
    const originalImage = sourceOriginalDataUrl || sourceImageDataUrl;
    const pdfSource = improvedImage || processedImageSource || originalImage;
    if (!pdfSource) {
    ...
    try {
      const progress = (stage: PdfExportProgress) => {
        setPdfExportStage(stage);
        setMessage(stage === "preparing" ? "Preparando imagem..." : stage === "creating" ? "Criando PDF..." : "Finalizando arquivo...");
      };
      // PDF is a raster export: always use the selected visual source, never
      // the generated CAD geometry or its SVG representation.
      const blob = await exportImageToPdf(pdfSource, { pageSize: pdfPageSize, orientation: pdfOrientation }, progress);
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_50%_-20%,#1d3428_0,#080c0b_42%)]">
    ...
    {hasSource && <section className="workspace-layout" style={{ "--controls-width": `${panel.size}px`, "--cad-width": `${cadPanel.size}px` } as React.CSSProperties}>
        ...
        <Section title="Pré-processamento" icon={<WandSparkles size={14} />}>
          ...
          {isAiEnhanceQuality(imageQuality) && <div className="mt-3 rounded-lg border border-[#33433a] bg-[#111914] p-2 text-[10px] text-[#aab8b0]">
            ...
            <div className="mt-2 grid grid-cols-2 gap-x-2 gap-y-1 leading-4">
              ...
              <span>Resolução final: {aiEnhanceMetrics.finalWidth.toLocaleString("pt-BR")} × {aiEnhanceMetrics.finalHeight.toLocaleString("pt-BR")}</span>
              <span>Melhoria aplicada: contraste + nitidez</span><span>Tempo: {aiEnhanceMetrics.processingMs} ms</span>
            </div>
          </div>}
      ...
      <aside className="cad-panel border-l border-[#26312c] bg-[#0d1210] p-4">
        {show3d && <div className="mb-5 grid gap-2 border-b border-[#26312c] pb-5"><div className="text-[10px] font-black uppercase tracking-[.12em] text-[#7f8e86]">Modo de visualização 3D</div><button type="button" role="radio" aria-checked={viewMode === "split-3d"} onClick={() => setViewMode("split-3d")} className={`flex items-center justify-between rounded-lg border px-3 py-2.5 text-left text-xs font-black ${viewMode === "split-3d" ? "border-[#b7f34a] bg-[#182019] text-[#b7f34a]" : "border-[#34413b] text-[#dce8e1]"}`}><span><span className="mr-2">{viewMode === "split-3d" ? "●" : "○"}</span>Painel lateral</span><span className="text-[9px] font-normal text-[#829087]">Editor + 3D</span></button><button type="button" role="radio" aria-checked={(viewMode as ViewMode) === "fullscreen-3d"} onClick={() => setViewMode("fullscreen-3d")} className={`flex items-center justify-between rounded-lg border px-3 py-2.5 text-left text-xs font-black ${(viewMode as ViewMode) === "fullscreen-3d" ? "border-[#b7f34a] bg-[#182019] text-[#b7f34a]" : "border-[#34413b] text-[#dce8e1]"}`}><span><span className="mr-2">{(viewMode as ViewMode) === "fullscreen-3d" ? "●" : "○"}</span>Tela cheia</span><span className="text-[9px] font-normal text-[#829087]">Somente 3D</span></button>{viewMode !== "editor" && <button type="button" onClick={() => setViewMode("editor")} className="flex items-center justify-center gap-2 rounded-lg border border-[#34413b] py-2.5 text-xs font-bold text-[#dce8e1]"><ArrowLeft size={14} /> Voltar ao Editor</button>}</div>}
    ...
    {pdfExportOpen && <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 px-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-3xl border border-[#54a9ff]/50 bg-[#101613] p-6 text-[#e8efeb] shadow-2xl shadow-black/50">
        <div className="flex items-start justify-between gap-4"><div><div className="text-xs font-black uppercase tracking-[.18em] text-[#8cc7ff]">Exportação</div><h3 className="mt-2 text-2xl font-black">Exportar imagem tratada em PDF</h3><p className="mt-2 text-sm leading-6 text-[#9caaa3]">A melhor imagem disponível será centralizada e ajustada sem distorção.</p></div><button type="button" onClick={() => setPdfExportOpen(false)} className="rounded-lg px-2 text-xl text-[#829087] hover:text-white" aria-label="Fechar exportação PDF">×</button></div>
