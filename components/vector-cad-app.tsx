...
export function VectorCadApp({ onUsageChange, initialData, onProjectChange, projectId, userId, draftClearSignal, persistenceStatus, lastSavedAt, projectVersion = 0, savedProjectVersion = 0 }: { onUsageChange?: (usage: UsageInfo) => void; initialData?: CadProjectData | null; onProjectChange?: (data: CadProjectData) => void; projectId?: string | null; userId?: string | null; draftClearSignal?: string; persistenceStatus?: "saved" | "saving" | "dirty" | "error"; lastSavedAt?: string | null; projectVersion?: number; savedProjectVersion?: number }) {
  ...
  const effectivePersistenceStatus = persistenceStatus || (projectId ? "saved" : "dirty");
  // Object references are recreated during hydration. The dashboard version is
  // the authoritative signal that the exact current document reached Supabase.
  const hasUnsavedChanges = !projectId || effectivePersistenceStatus !== "saved" || projectVersion !== savedProjectVersion;
  const blocked3dMessage = effectivePersistenceStatus === "saving"
