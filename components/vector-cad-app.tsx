...
export function VectorCadApp({ onUsageChange, initialData, onProjectChange, projectId, userId, draftClearSignal, persistenceStatus, lastSavedAt }: { onUsageChange?: (usage: UsageInfo) => void; initialData?: CadProjectData | null; onProjectChange?: (data: CadProjectData) => void; onPrepare3dProject?: (data: CadProjectData) => Promise<string | null>; projectId?: string | null; userId?: string | null; draftClearSignal?: string; persistenceStatus?: "saved" | "saving" | "dirty" | "error"; lastSavedAt?: string | null }) {
  ...
  const hasUnsavedChanges = !projectId || effectivePersistenceStatus !== "saved" || localDraftDirty || !initialData?.document || Boolean(doc && initialData.document && doc !== initialData.document);
